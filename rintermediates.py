...
#from bagh_code.fortran_lib.rhf_ri import contract_3p_ip_ri
...
def cc_Foo(t1, t2, eris):
    ...
    Fki = foo.copy()
    Fki +=2*np.einsum('kcld,ilcd->ki', eris.ovov, t2)
    Fki -= np.einsum('kdlc,ilcd->ki', eris.ovov, t2)
    #Fki += 2*np.einsum('kcld,ic,ld->ki', eris.ovov, t1, t1)
    tmp = np.einsum('kcld,ld->kcld',eris.ovov, t1)
    tmp = np.einsum('kcld,ic->ki',tmp, t1)
    Fki +=2*tmp
    tmp = np.einsum('kdlc,ld->kdlc',eris.ovov, t1)
    tmp = np.einsum('kdlc,ic->ki',tmp, t1)
    Fki -= tmp
    #Fki -= np.einsum('kdlc,ic,ld->ki', eris.ovov, t1, t1)
    return Fki
...
def Loo(t1, t2, eris):
    ...
    Lki += 2*np.einsum('lcki,lc->ki', eris.ovoo, t1)
    Lki -=   np.einsum('kcli,lc->ki', eris.ovoo, t1)
...
def Lvv(t1, t2, eris):
    ...
    if (eris.incore < 4 and eris.df):
        mtemp=2 * lib.einsum('dkm,kd->m', eris.Lov.T, t1)
        Lac +=lib.einsum('m,mac->ac', mtemp, eris.Lvv)
        mtemp=None
        #metmp=lib.einsum('kd,mad->mak', t1,eris.Lvv)
        #Lac -= lib.einsum('mck,mak->ac',eris.Lov,mtemp)
        mtemp=lib.einsum('kd,mkc->mdc', t1,eris.Lov)
        Lac -= lib.einsum('mdc,mad->ac', mtemp, eris.Lvv)
        mtemp = None
    else:
...
def W2ovvo(t1, t2, eris):
    ...
    if (eris.incore < 4 and eris.df):
       temp_mai = lib.einsum('mad,id->mai', eris.Lvv, t1)
       Wkaci += lib.einsum('ckm,mai->kaci', eris.Lov.T, temp_mai)
       temp_mai=None
    else:
     eris_ovvv = np.asarray(eris.get_ovvv())
...
def W2ovov(t1, t2, eris):
    ...
    if (eris.incore < 4 and eris.df):
     tmp_kim = lib.einsum('ic,mkc->ikm', t1, eris.Lov)
     Wkbid += lib.einsum('ikm,mbd->kbid', tmp_kim, eris.Lvv)
     tmp_kim = None
    else:
     eris_ovvv = np.asarray(eris.get_ovvv())
...
def Wovoo(t1, t2, eris):
    ...
    if (eris.incore < 4 and eris.df):
        tautemp = np.einsum('jd,ic->jidc', t1, t1)
        tautemp+=t2
        nocc, nvir = t1.shape
        naux=eris.naux
        print(naux)
        #ttemp = lib.einsum('jidc,mkc->ijkdm', tautemp, eris.Lov)
        #Wkbij += lib.einsum('ijkdm,mbd->kbij', ttemp, eris.Lvv)
       # ttemp=contract_3p_ip_ri(tautemp.T, eris.Lvv.T,eris.Lov.T, nocc, nvir, naux)
        #Wkbij+=ttemp
        #Wkbij=contract_ip_ri_3p(tautemp,Wkbij, eris.Lvv,eris.Lov, nocc)
        #ttemp=None
       # tautemp=None
